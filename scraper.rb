...
a.get(url) do |page|
  ...
  variable_name = js_expr.match(/([a-z]+)=(.*);document\.cookie=(.*); location\.reload\(\);/m)[1]
  s_expr = js_expr.match(/([a-z]+)=(.*);document\.cookie=(.*); location\.reload\(\);/m)[2]
  d_expr = js_expr.match(/([a-z]+)=(.*);document\.cookie=(.*); location\.reload\(\);/m)[3]

  s = evaluate_expression(s_expr)
  d_expr = d_expr.gsub(" #{variable_name} ", " '#{s}' ")
